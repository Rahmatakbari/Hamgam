...
function money(n) { return toFa(group(n)) + ' ' + DB.settings.currency; }
function qtyTxt(n) { return toFa(group(n)); }
function signedMoney(n) {
  var v = m2(n);
  var color = v < 0 ? 'var(--danger)' : v > 0 ? 'var(--ink)' : 'var(--muted)';
  var sign = v < 0 ? '-' : '';
  return '<span class="tnum" style="color:' + color + ';direction:ltr;display:inline-block">' + sign + toFa(group(Math.abs(v))) + '</span> ' + esc(DB.settings.currency);
}
...
function blankDB() {
  return {
    settings: {
      shop: 'فروشگاه هم‌گام', currency: 'افغانی', phone: '', address: '',
...
var ACT = {
  ...
  accountMenu: function (el) {
    ...
    var menu = [
      ...
      { label: 'ثبت آوردگی', act: 'accPayForm', icon: 'money' },
      { label: 'ثبت بردگی', act: 'accDebtForm', icon: 'cart' },
      { sep: true },
    ...
    showContextMenu(el, menu, function(act) {
      if (act === 'openAccount') showAccount(accId);
      ...
      else if (act === 'accPayForm') {
        var fakeEl = { dataset: { id: accId } };
        ACC_HANDLERS.accPayForm(fakeEl);
      }
      else if (act === 'accDebtForm') {
        var fakeEl = { dataset: { id: accId } };
        ACC_HANDLERS.accDebtForm(fakeEl);
      }
        ...
        var txt = '📋 خلاصه حساب\n' +
          ...
          'مانده: ' + group(b) + ' ' + DB.settings.currency + '\n' +
...
/* فرم ورود کالا (خرید) */
function receiptForm(preselectSupplierId) {
  receiptItems = [];
  var opts = '';
...
function showMovements(pid) {
  ...
  sheet({ title: 'تاریخچه حرکت — ' + (p ? p.name : ''), body: h });
}
...
function buildWhatsappLink(customer, amount) {
  ...
  var message = template
    ...
    .replace(/\{amount\}/g, group(amount) + ' ' + DB.settings.currency);
...
function init() {
  ...
  window.addEventListener('popstate', function () { if (sheetRoot.innerHTML) { closeSheet(); history.pushState(null, ''); } });
