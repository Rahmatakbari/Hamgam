...
const AuthStorage = {
  ...
  activateApp: function(activationPassword) {
    ...
    // hash کردن رمز فعال‌سازی برای امنیت بیشتر
    const hash = this.hashPassword(activationPassword);
    localStorage.setItem(this.keys.ACTIVATION_PASSWORD, hash);
  },
  
  // بررسی رمز فعال‌سازی
  verifyActivationPassword: function(password) {
    const storedHash = localStorage.getItem(this.keys.ACTIVATION_PASSWORD);
    if (!storedHash) {
      // fallback برای داده‌های قدیمی (plain text)
      const storedPassword = '1234';
      if (password === storedPassword) {
        // migration: hash کردن رمز قدیمی
        this.activateApp(password);
        return true;
      }
      return false;
    }
...
function handleLogin() {
  ...
  if (AuthStorage.verifyUserPassword(password)) {
    // ورود موفق
    localStorage.setItem('hamgam_logged_in', 'true');
...
async function handleFingerprint() {
  ...
  if (!window.PublicKeyCredential) {
    toast('مرورگر شما از اثر انگشت پشتیبانی نمی‌کند', 'warn');
    return;
  }
  
  try {
    // بررسی اینکه آیا کاربر قبلاً biometric را ثبت کرده یا نه
    const available = await PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable();
    
    if (!available) {
      toast('اثر انگشت یا Face ID در دستگاه شما فعال نیست', 'warn');
      return;
    }
    
    // بررسی اینکه آیا credential قبلاً ثبت شده یا نه
    const credentialId = localStorage.getItem('hamgam_biometric_credential');
    
    if (!credentialId) {
      // ثبت biometric جدید
      if (confirm('آیا می‌خواهید اثر انگشت یا Face ID را برای ورود سریع ثبت کنید؟')) {
        await registerBiometric();
      }
    } else {
      // احراز هویت با biometric
      await authenticateBiometric(credentialId);
    }
  } catch (error) {
    console.error('Biometric error:', error);
    toast('خطا در احراز هویت بیومتریک', 'warn');
  }
}
...
async function registerBiometric() {
  try {
    ...
    if (credential) {
      ...
      localStorage.setItem('hamgam_biometric_credential', credentialId);
      toast('✅ اثر انگشت با موفقیت ثبت شد', 'ok');
    }
  } catch (error) {
    console.error('Registration error:', error);
    toast('خطا در ثبت اثر انگشت', 'warn');
  }
}
...
async function authenticateBiometric(credentialId) {
  try {
    ...
    if (assertion) {
      ...
      localStorage.setItem('hamgam_logged_in', 'true');
      document.getElementById('authContainer').style.display = 'none';
      toast('✅ ورود با اثر انگشت موفق بود', 'ok');
    ...
    if (error.name === 'NotAllowedError') {
      toast('احراز هویت لغو شد', 'warn');
    } else {
      toast('خطا در احراز هویت', 'warn');
    }
  }
}
...
function handleForgotPassword() {
  if (confirm('آیا مطمئن هستید؟ این عملیات تمام داده‌های برنامه را پاک می‌کند.')) {
    ...
    localStorage.removeItem('hamgam_logged_in');
    location.reload();
  }
}
...
function initAuth() {
  try {
    ...
    // اگر برنامه قبلاً فعال شده و کاربر وارد شده، container را مخفی کن و برنامه را راه‌اندازی کن
    if (state === AuthStates.READY) {
      const isLoggedIn = localStorage.getItem('hamgam_logged_in') === 'true';
      if (isLoggedIn) {
...
function inRange(iso, range) {
  ...
  if (range === 'month') {
    ...
    var lastDay = jalaliMonthLength(jalaliToday.jy, jalaliToday.jm);
    var lastDayOfMonth = toGregorian(jalaliToday.jy, jalaliToday.jm, lastDay);
    var nextMonth = new Date(lastDayOfMonth);
    nextMonth.setDate(nextMonth.getDate() + 1);
    
    return d >= firstDayOfMonth && d < nextMonth;
  }
...
/**
 * تبدیل Date به ISO string محلی (با timezone محلی)
 * جایگزین toISOString() که UTC است
 */
function toLocalISOString(date) {
  var d = date || new Date();
...
var _appInitialized = false; // جلوگیری از اجرای دوباره init
...
function viewAnalysis() {
  ...
  for (var i = days - 1; i >= 0; i--) {
    ...
    var dayStr = toLocalDateStr(d);
    var daySales = 0;
    DB.sales.forEach(function(s) {
      if (toLocalDateStr(new Date(s.date)) === dayStr) daySales += s.total;
    });
  ...
  for (var i = days - 1; i >= 0; i--) {
    ...
    DB.treasury.forEach(function(t) {
      if (toLocalDateStr(new Date(t.date)) !== dayStr) return;
...
function viewSettings() {
  ...
  h += '<div class="card">' +
    ...
    '<button class="btn soft block sm danger" style="margin-top:8px" onclick="handleLogout()">🚪 خروج از حساب کاربری</button>' +
    '</div>';
...
var ACT = {
  ...
  saveEditAccount: function (el) {
    ...
    var newType = $('#editAccType').value;
    
    // بررسی تغییر نوع حساب
    if (acc.type !== newType) {
      // بررسی سابقه مالی
      var hasHistory = false;
      
      if (acc.type === 'customer' || acc.type === 'supplier') {
        var entityId = acc.oldId || acc.id;
        
        // بررسی فاکتورها
        if (acc.type === 'customer') {
          hasHistory = DB.sales.some(function(s) { return s.customerId === entityId; });
        } else {
          hasHistory = DB.purchases.some(function(p) { return p.supplierId === entityId; });
        }
        
        // بررسی پرداخت‌ها
        if (!hasHistory && acc.type === 'customer') {
          hasHistory = DB.payments.some(function(p) { return p.customerId === entityId; });
        } else if (!hasHistory && acc.type === 'supplier') {
          hasHistory = DB.supplierPayments.some(function(p) { return p.supplierId === entityId; });
        }
        
        // بررسی adjustments
        if (!hasHistory) {
          hasHistory = DB.accountAdjustments.some(function(a) {
            return (a.kind === 'cust' && a.customerId === entityId) ||
                   (a.kind === 'supp' && a.supplierId === entityId);
          });
        }
      }
      
      if (hasHistory) {
        return toast('این حساب دارای سابقه مالی است و نوع آن قابل تغییر نیست', 'warn');
      }
    }
  ...
  saveEditTransaction: function (el) {
    ...
    if (kind === "pay") {
      ...
      if (payment) {
        // ذخیره مقادیر قبلی برای به‌روزرسانی خزانه
        var oldAmount = payment.amount;
        var oldDate = payment.date;
        var oldNote = payment.note;
        
        payment.amount = isIn ? amount : -amount;
        payment.note = note;
        payment.date = selectedDate.toISOString();
        
        // به‌روزرسانی رکورد خزانه مرتبط (اگر با خزانه ثبت شده)
        if (payment.withTreasury !== false) {
          var treasuryRecord = DB.treasury.find(function(t) {
            return t.note === oldNote && 
                   Math.abs(t.amount) === Math.abs(oldAmount) &&
                   t.date === oldDate &&
                   ((isIn && t.type === 'in') || (!isIn && t.type === 'out'));
          });
          
          if (treasuryRecord) {
            treasuryRecord.amount = isIn ? amount : -amount;
            treasuryRecord.note = note;
            treasuryRecord.date = selectedDate.toISOString();
          }
        }
      }
    } else if (kind === "spay") {
      ...
      if (spay) {
        // ذخیره مقادیر قبلی برای به‌روزرسانی خزانه
        var oldAmount = spay.amount;
        var oldDate = spay.date;
        var oldNote = spay.note;
        ...
        // به‌روزرسانی رکورد خزانه مرتبط (اگر با خزانه ثبت شده)
        if (spay.withTreasury !== false) {
          var treasuryRecord = DB.treasury.find(function(t) {
            return t.note === oldNote && 
                   Math.abs(t.amount) === Math.abs(oldAmount) &&
                   t.date === oldDate &&
                   ((isIn && t.type === 'out') || (!isIn && t.type === 'in'));
          });
      ...
      if (adj) {
        // ذخیره مقادیر قبلی برای به‌روزرسانی خزانه
        var oldDelta = adj.delta;
        var oldDate = adj.date;
        var oldNote = adj.note;
        ...
        // به‌روزرسانی رکورد خزانه مرتبط (اگر با خزانه ثبت شده)
        if (adj.withTreasury !== false) {
          var treasuryRecord = DB.treasury.find(function(t) {
            return t.note === oldNote && 
                   Math.abs(t.amount) === Math.abs(oldDelta) &&
                   t.date === oldDate;
          });
  ...
  saveEditSale: function (el) {
    ...
    // 3. اگر مشتری تغییر کرده، پرداخت‌های مرتبط را هم به‌روزرسانی کن
    if (oldCustomerId !== newCustomerId) {
      var invoiceNote = 'پرداخت فاکتور ' + s.no;
      DB.payments.forEach(function(p) {
        if (p.customerId === oldCustomerId && p.note === invoiceNote) {
      ...
      if (act === 'edit') {
        ...
        confirmBox('حذف تراکنش', 'آیا از حذف این تراکنش مطمئن هستید؟', function () {
          ...
          if (kind === 'pay') {
            var payment = DB.payments.find(function(p) { return p.id === tid; });
            if (payment) {
              // حذف رکورد خزانه مرتبط (اگر با خزانه ثبت شده)
              if (payment.withTreasury !== false) {
                DB.treasury = DB.treasury.filter(function(t) {
                  return !(t.note === payment.note && 
                           Math.abs(t.amount) === Math.abs(payment.amount) &&
                           t.date === payment.date &&
                           t.type === 'in');
                });
              }
            ...
            if (adj) {
              ...
              if (adj.withTreasury !== false) {
                DB.treasury = DB.treasury.filter(function(t) {
                  return !(t.note === adj.note && 
                           Math.abs(t.amount) === Math.abs(adj.delta) &&
                           t.date === adj.date);
                });
              }
              // حذف adjustment
        ...
        confirmBox('حذف تراکنش', 'آیا از حذف این تراکنش مطمئن هستید؟', function () {
          ...
          if (kind === 'spay') {
            ...
            if (spay) {
              ...
              if (spay.withTreasury !== false) {
                DB.treasury = DB.treasury.filter(function(t) {
                  return !(t.note === spay.note && 
                           Math.abs(t.amount) === Math.abs(spay.amount) &&
                           t.date === spay.date &&
                           t.type === 'out');
                });
              }
            ...
            var sadj = DB.accountAdjustments.find(function(a) { return a.id === tid; });
            if (sadj) {
              // حذف رکورد خزانه مرتبط (اگر با خزانه ثبت شده)
              if (sadj.withTreasury !== false) {
                DB.treasury = DB.treasury.filter(function(t) {
                  return !(t.note === sadj.note && 
                           Math.abs(t.amount) === Math.abs(sadj.delta) &&
                           t.date === sadj.date);
                });
              }
              // حذف adjustment
    ...
    showContextMenu(el, menu, function(act) {
      if (act === 'openAccount') showAccount(accId);
        ...
        requirePin(function() {
          ...
          if (acc.type === 'customer') {
            hasHistory = DB.sales.some(function(s) { return s.customerId === entityId; }) ||
                         DB.payments.some(function(p) { return p.customerId === entityId; }) ||
                         DB.accountAdjustments.some(function(a) { return a.kind === 'cust' && a.customerId === entityId; });
          } else if (acc.type === 'supplier') {
            hasHistory = DB.purchases.some(function(p) { return p.supplierId === entityId; }) ||
                         DB.supplierPayments.some(function(p) { return p.supplierId === entityId; }) ||
                         DB.accountAdjustments.some(function(a) { return a.kind === 'supp' && a.supplierId === entityId; });
          }
          
          if (hasHistory) {
            // حساب دارای سابقه مالی است → غیرفعال کن
            confirmBox('غیرفعال کردن حساب', 'این حساب دارای سابقه مالی است و قابل حذف نیست. آیا می‌خواهید آن را غیرفعال کنید؟ حساب غیرفعال در لیست فعال نمایش داده نمی‌شود ولی سوابق مالی حفظ می‌شود.', function() {
              acc.disabled = true;
              save(); render();
              toast('حساب غیرفعال شد', 'ok');
            });
          } else {
            // حساب بدون سابقه مالی → حذف کن
            confirmBox('حذف حساب', 'آیا از حذف این حساب مطمئن هستید؟ این حساب هیچ سابقه مالی ندارد.', function() {
